...
function nextAvailableId(projects) { const used = new Set(projects.map((project) => Number(project.id)).filter((id) => Number.isSafeInteger(id) && id > 0)); let id = 1; while (used.has(id)) id += 1; return id; }
...
function Editor({ project, projects, close, saved, notice, remove }) {
    ...
    const submit = async (published) => { const next = { ...form, id: String(project?.id ?? form.id), translationKey: project?.translationKey ?? form.translationKey, published: published ?? form.published }; const nextErrors = validate(next, projects, project?.id); setErrors(nextErrors); if (Object.keys(nextErrors).length) { notice('İşaretli alanları düzeltin.', true); requestAnimationFrame(() => document.querySelector('[aria-invalid="true"]')?.focus()); return; } setSaving(true); try { const payload = asProject(next); const savedProject = await projectService.saveProject(payload, project?.docId); markSaved(next); saved(savedProject); notice(payload.published ? 'Proje yayınlandı.' : 'Taslak kaydedildi.'); } catch (error) { notice(`Kaydetme başarısız: ${error.message}`, true); } finally { setSaving(false); } };
...
export default function Admin() {
    ...
    const sameRecord = (first, second) => String(first.docId ?? first.id) === String(second.docId ?? second.id);
    ...
    const archive = async (project) => { if (!window.confirm(`“${project.title}” arşivlensin mi?`)) return; try { await projectService.setArchived(project, true); setProjects((list) => list.map((item) => sameRecord(item, project) ? { ...item, archived: true } : item)); notice('Proje arşivlendi.'); } catch (error) { notice(`Arşivleme başarısız: ${error.message}`, true); } };
    const erase = async (project) => { try { await projectService.deleteProject(project); setProjects((list) => list.filter((item) => !sameRecord(item, project))); setSelected(null); notice('Proje kalıcı olarak silindi.'); } catch (error) { notice(`Silme başarısız: ${error.message}`, true); } };
