...
import { nextAvailableId } from '../utils/projectIdentity';
...
function validate(form, projects, editingId, t) { const errors = {}; REQUIRED.forEach((key) => { if (!form[key]?.trim()) errors[key] = 'Bu alan zorunludur.'; }); if (!STATUSES.includes(form.status)) errors.status = 'Geçerli durum seçin.'; if (projects.some((p) => String(p.id) === form.id && String(p.id) !== String(editingId))) errors.id = 'Bu ID zaten kullanılıyor.'; if (!/^[A-Za-z][A-Za-z0-9_]{0,79}$/.test(form.translationKey)) errors.translationKey = t('admin.invalidTranslationKey'); if (projects.some((p) => p.translationKey === form.translationKey && String(p.id) !== String(editingId))) errors.translationKey = 'Bu anahtar zaten kullanılıyor.'; ['link', 'demoLink'].forEach((key) => { if (form[key] && !/^https?:\/\/.+/.test(form[key])) errors[key] = 'Geçerli bir http(s) URL girin.'; }); const imageError = validateImageValue(form.image); if (imageError) errors.image = imageError; return errors; }
...
const projectRecordId = (project) => String(project?.docId ?? project?.id);
...
function Editor({ project, projects, close, saved, notice, remove, t }) {
    const initial = useMemo(() => formFor(project, projects), [project, projects]);
    const { form, setForm, dirty, markSaved, discard } = useProjectDraft(initial, project ? projectRecordId(project) : `new-${initial.id}`);
    ...
    const asProject = (next) => { const lines = (value) => value.split('\n').map((item) => item.trim()).filter(Boolean); const payload = { ...next, id: Number(project?.id ?? next.id), translationKey: project?.translationKey ?? next.translationKey, order: project?.order ?? projects.length, tags: next.tagsInput.split(',').map((item) => item.trim()).filter(Boolean), learningsTr: lines(next.learningsTrInput), learningsEn: lines(next.learningsEnInput), subtitle: next.subtitleTr || next.subtitleEn, description: next.descriptionTr || next.descriptionEn, role: next.roleTr || next.roleEn, outcome: next.outcomeTr || next.outcomeEn }; delete payload.tagsInput; delete payload.learningsTrInput; delete payload.learningsEnInput; return payload; };
    const submit = async (published) => { const next = { ...form, id: String(project?.id ?? form.id), translationKey: project?.translationKey ?? form.translationKey, published: published ?? form.published }; const nextErrors = validate(next, projects, project?.id, t); setErrors(nextErrors); if (Object.keys(nextErrors).length) { notice('İşaretli alanları düzeltin.', true); requestAnimationFrame(() => document.querySelector('[aria-invalid="true"]')?.focus()); return; } setSaving(true); try { const payload = asProject(next); const savedProject = project ? await projectService.saveProject(payload, project.docId) : await projectService.createProject(payload, projects); markSaved(next); saved(savedProject); notice(payload.published || project?.published ? t('admin.projectSavedPendingDeployment') : t('admin.draftSaved')); } catch (error) { const message = error.code === 'project/translation-key-already-exists' ? t('admin.translationKeyAlreadyUsed') : error.message; notice(`Kaydetme başarısız: ${message}`, true); } finally { setSaving(false); } };
    ...
    return <section className="metadata-card admin-editor"><header className="admin-editor-header"><div><span className="admin-eyebrow">Proje editörü</span><h2>{project ? project.title : 'Yeni proje'}</h2><div className="admin-editor-state"><span className={`admin-publish-state ${form.published ? 'live' : ''}`}>{form.published ? 'Yayında' : 'Taslak'}</span>{dirty && <span className="admin-unsaved">Kaydedilmemiş değişiklik</span>}</div></div><button className="btn admin-secondary" type="button" onClick={safeClose}>Editörü kapat</button></header><div className="admin-tabs" role="tablist" aria-label="Proje düzenleme bölümleri">{[['general','Genel'],['content','İçerik'],['media','Medya'],['links','Linkler'],['advanced','Gelişmiş']].map(([key, label]) => <button type="button" role="tab" aria-selected={section === key} className={section === key ? 'active' : ''} onClick={() => setSection(key)} key={key}>{label}</button>)}</div><form className="admin-form" onSubmit={(e) => { e.preventDefault(); submit(); }}>
        ...
        {section === 'advanced' && <div className="admin-grid"><Field label="ID" error={errors.id}><input inputMode="numeric" value={form.id} readOnly /></Field><Field label="translationKey" error={errors.translationKey}><input value={form.translationKey} onChange={(e) => set('translationKey', e.target.value)} readOnly={Boolean(project)} /></Field></div>}
...
export default function Admin() {
    const navigate = useNavigate(); const { t } = useLanguage(); const [user, setUser] = useState(null); const [authLoading, setAuthLoading] = useState(true); const [loading, setLoading] = useState(false); const [projects, setProjects] = useState([]); const [selected, setSelected] = useState(null); const [loadError, setLoadError] = useState(''); const [status, setStatus] = useState(null); const [showArchived, setShowArchived] = useState(false); const [ordering, setOrdering] = useState(false); const [orderDirty, setOrderDirty] = useState(false);
    ...
    const sameRecord = (first, second) => projectRecordId(first) === projectRecordId(second);
    const upsert = (saved) => { setProjects((list) => { const i = list.findIndex((project) => sameRecord(project, saved)); return i < 0 ? [...list, saved] : list.map((project, index) => index === i ? saved : project); }); setSelected(saved); };
    const archive = async (project) => { if (!window.confirm(`“${project.title}” arşivlensin mi?`)) return; try { await projectService.setArchived(project, true); setProjects((list) => list.map((item) => sameRecord(item, project) ? { ...item, archived: true } : item)); notice(t('admin.projectArchivedPendingDeployment')); } catch (error) { notice(`Arşivleme başarısız: ${error.message}`, true); } };
    const erase = async (project) => { try { await projectService.deleteProject(project); setProjects((list) => list.filter((item) => !sameRecord(item, project))); setSelected(null); notice(t('admin.projectDeletedPendingDeployment')); } catch (error) { notice(`Silme başarısız: ${error.message}`, true); } };
    const move = (projectId, targetId) => { setProjects((list) => { const next = [...list]; const index = next.findIndex((item) => projectRecordId(item) === String(projectId)); const targetIndex = next.findIndex((item) => projectRecordId(item) === String(targetId)); if (index < 0 || targetIndex < 0 || index === targetIndex) return list; const [moved] = next.splice(index, 1); next.splice(targetIndex, 0, moved); return next; }); setOrderDirty(true); };
    const finishProjectDrag = (commit = false) => {
        ...
        if (commit && session.targetIndex !== session.originalIndex) {
            setProjects((list) => {
                const next = [...list];
                const currentIndex = next.findIndex((item) => projectRecordId(item) === session.projectId);
                const targetId = session.rowIds[session.targetIndex];
                const targetIndex = next.findIndex((item) => projectRecordId(item) === targetId);
    ...
    const draggedProject = projects.find((project) => projectRecordId(project) === draggingId);
    ...
    const deploymentNotice = <p className="admin-order-hint" role="note">{t('admin.staticDeploymentNotice')}</p>;
    if (selected) return <main className="container admin-page" aria-busy={loading}>{toast}{header}{deploymentNotice}<section className="admin-workspace editing"><aside className="admin-list-pane" aria-label="Proje listesi"><div className="admin-pane-heading"><h2>Projeler</h2><span>{visible.length}</span></div>{visible.map((project) => <button type="button" className={`admin-project-picker ${projectRecordId(project) === projectRecordId(selected) ? 'active' : ''}`} onClick={() => setSelected(project)} key={projectRecordId(project)}><ProjectThumbnail project={project} compact /><span className="admin-picker-copy"><strong>{project.title}</strong><small>{project.published ? 'Yayında' : 'Taslak'}</small></span></button>)}</aside><Editor project={selected.isNew ? null : selected} projects={projects} close={() => setSelected(null)} saved={upsert} notice={notice} remove={erase} t={t} /></section></main>;

    return <main className="container admin-page" aria-busy={loading}>{toast}{header}{deploymentNotice}{loadError ? <section className="admin-error-panel" role="alert"><p>Projeler yüklenemedi: {loadError}</p><button className="btn admin-secondary" onClick={load}>Yeniden dene</button></section> : <><section className="admin-overview" aria-label="Proje özeti"><div><strong>{projects.length}</strong><span>Toplam proje</span></div><div><strong>{liveCount}</strong><span>Yayında</span></div><div><strong>{draftCount}</strong><span>Taslak</span></div><div><strong>{archivedCount}</strong><span>Arşivde</span></div><div className={incompleteCount ? 'attention' : ''}><strong>{incompleteCount}</strong><span>İçeriği eksik</span></div></section><section className="admin-list-panel"><div className="admin-list-heading"><div><span className="admin-eyebrow">İçerik kataloğu</span><h2>Projeler</h2></div><div className="admin-list-tools"><button className={`btn admin-secondary ${showArchived ? 'active' : ''}`} onClick={() => setShowArchived((v) => !v)}>{showArchived ? 'Arşivleri gizle' : 'Arşivleri göster'}</button><button className={`btn admin-secondary ${ordering ? 'active' : ''}`} onClick={() => { setOrdering((v) => !v); setShowArchived(true); finishProjectDrag(); }}>{ordering ? 'Sıralamayı kapat' : 'Sıralama modu'}</button>{ordering && orderDirty && <button className="btn primary" onClick={saveOrder}>Sıralamayı kaydet</button>}</div></div>{ordering && <p className="admin-order-hint" role="status">Tutamacı basılı tutup projeyi istediğiniz konuma sürükleyin. İmleç liste dışına çıksa da sürükleme devam eder; vazgeçmek için Esc’e basın. Klavye için ok düğmelerini kullanabilirsiniz.</p>}<div className={`admin-list ${draggingId ? 'is-dragging' : ''}`}>{loading ? <p className="admin-empty" role="status">Projeler yükleniyor…</p> : visible.length === 0 ? <p className="admin-empty">{showArchived ? 'Arşivde proje yok.' : 'Proje yok.'}</p> : visible.map((project, index) => { const complete = project.subtitleTr && project.subtitleEn && project.descriptionTr && project.descriptionEn; const projectId = projectRecordId(project); let dragShift = 0; if (dragLayout && index !== dragLayout.originalIndex) { if (dragLayout.originalIndex < dragTargetIndex && index > dragLayout.originalIndex && index <= dragTargetIndex) dragShift = -dragLayout.draggedHeight; if (dragLayout.originalIndex > dragTargetIndex && index >= dragTargetIndex && index < dragLayout.originalIndex) dragShift = dragLayout.draggedHeight; } return <article className={`admin-project-row ${ordering ? 'ordering' : ''} ${draggingId === projectId ? 'dragging' : ''} ${dragShift ? 'drag-shifted' : ''}`} style={dragShift ? { transform: `translate3d(0, ${dragShift}px, 0)` } : undefined} data-project-id={projectId} key={projectId}>{ordering && <button type="button" className="admin-drag-handle" aria-label={`${project.title} projesini basılı tutup sürükleyerek sırala`} data-dragging={draggingId === projectId || undefined} onPointerDown={(event) => startProjectDrag(event, projectId)}><span aria-hidden="true">⠿</span></button>}<ProjectThumbnail project={project} /><div className="admin-project-copy"><div className="admin-project-title"><strong>{project.title}</strong><PublishBadge project={project} /></div><span>{project.category || 'Other'} · {project.status}</span><small className={complete ? '' : 'incomplete'}>TR/EN içerik {complete ? 'tamam' : 'eksik'}</small></div>{ordering && <div className="admin-order-actions"><button className="admin-icon-button" aria-label={`${project.title} yukarı taşı`} disabled={index === 0} onClick={() => move(projectId, projectRecordId(visible[index - 1]))}>↑</button><button className="admin-icon-button" aria-label={`${project.title} aşağı taşı`} disabled={index === visible.length - 1} onClick={() => move(projectId, projectRecordId(visible[index + 1]))}>↓</button></div>}<div className="admin-row-actions"><button className="btn admin-secondary" onClick={() => setSelected(project)}>Düzenle</button>{!project.archived && <button className="btn admin-secondary" onClick={() => archive(project)}>Arşivle</button>}<button className="btn admin-danger-secondary" onClick={() => setSelected(project)}>Sil…</button></div></article>; })}</div></section></>}</main>;
}
