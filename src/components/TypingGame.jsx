...
function getComment(wpm, t) {
    const ratio = wpm / PERSONAL_RECORD;
    const key = wpm >= PERSONAL_RECORD ? 'beat' : ratio >= 0.85 ? 'near' : ratio >= 0.65 ? 'good' : ratio >= 0.45 ? 'decent' : 'warmup';
...
export default function TypingGame() {
    ...
    const hasComparableRecord = duration === 15;
    ...
    return (
        <section className="tg-shell" aria-label={t('typingGame.label')}>
            <div className="tg-header">
                ...
                {duration === 15 && <div className="tg-record-badge"><span className="tg-record-key">{t('typingGame.record')}</span><span className="tg-record-val">{PERSONAL_RECORD} wpm</span></div>}
            </div>
            ...
            {phase === 'playing' && <>
                ...
                <input ref={inputRef} className={`tg-input${isCurrentWrong ? ' input-wrong' : ''}`} value={typed} onChange={handleInput} placeholder={t('typingGame.inputPlaceholder')} aria-label={t('typingGame.inputLabel')} autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck={false} />
            </>}
            {phase === 'done' && <div className="tg-result" aria-live="polite">
                ...
                <p className="tg-result-comment">{hasComparableRecord ? getComment(wpm, t) : t('typingGame.result.complete').replace('{wpm}', wpm)}</p>
                {hasComparableRecord && <div className="tg-result-compare">
                    <div className="tg-compare-item"><span className="tg-compare-label">{t('typingGame.you')}</span><div className="tg-compare-bar-wrap"><div className="tg-compare-bar you" style={{ width: `${Math.min(100, (wpm / (PERSONAL_RECORD * 1.2)) * 100)}%` }} /></div><span className="tg-compare-val">{wpm}</span></div>
                    <div className="tg-compare-item"><span className="tg-compare-label">{t('typingGame.me')}</span><div className="tg-compare-bar-wrap"><div className="tg-compare-bar me" style={{ width: `${Math.min(100, (PERSONAL_RECORD / (PERSONAL_RECORD * 1.2)) * 100)}%` }} /></div><span className="tg-compare-val">{PERSONAL_RECORD}</span></div>
                </div>}
                <button ref={restartButtonRef} className="tg-start-btn tg-restart-btn" type="button" onClick={startGame}>{t('typingGame.restart')}</button>
